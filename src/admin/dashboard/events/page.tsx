...
export default function AdminEventsPage() {
    ...
    const handleListSave = (listName: keyof ContentArrays, item: Event) => {
        setContent(prev => {
            ...
            const newItem = { ...item };
            if (newItem.id === 0) {
                const upcomingIds = prev.upcomingEvents.map(e => e.id);
                const pastIds = prev.pastEvents.map(e => e.id);
                newItem.id = getNewId([...upcomingIds, ...pastIds, ...prev.cslClasses.map(c => c.id), ...prev.resources.map(r => r.id)]);
            }
